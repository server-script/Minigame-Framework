...
local function Merge(t1, t2)
	local endTable = {}
	for _, element in pairs(t1) do
		table.insert(endTable, element)
	end
	for _, element in pairs(t2) do
		table.insert(endTable, element)
	end
	return endTable
end
...
local function MergeDictionaries(t1, t2)
	local endTable = {}
	for key, element in pairs(t1) do
		endTable[key] = element
	end
	for key, element in pairs(t2) do
		endTable[key] = element
	end
	return endTable
end
...
TableUtil.GetFirstElement = GetFirstElement
TableUtil.GetFirstKey = GetFirstKey
TableUtil.Merge = Merge
TableUtil.MergeDictionaries = MergeDictionaries
